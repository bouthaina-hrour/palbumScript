...
#uses exiftool to create hiearchy in the output directory containing years and every year contains dates 
...
function createHieararchy(){	
	exiftool  -o . -d "$1"/%Y/%Y_%m_%d  "-directory<filemodifydate" "-directory<datetimeoriginal"  "-directory<createdate" .
	cd "$1"
}
function name_images(){
	#$1 is the name of directory where images needed to be renamed
	for photo in $(find $1 -name '*jpg')
	do
	if [[ $photo != *-* ]] 
	then
	path=${photo%/*}/
...
function display_thumbnails(){
	...
	do  
		...
		echo '<a target="_blank" href="../'"${dir_name}"'/../'"${actual_image}"'.jpg">'>>$1/index.html	
